...
3. 优化了消息处理阻塞，增加了响应速度，减少可能出现的卡死情况

## 注意事项

由于该插件的运作逻辑，导致token消耗会**非常非常大**（在一个一天发2k条消息的群聊中使用，一天约消耗20Mtoken），使用请注意。  
~~也许在之后会优化一下token使用。~~  
目前更改了feedback逻辑，以降低智能的代价大幅降低了token使用。  
之后可能会再度优化feedback逻辑，使其更加智能。
